...
const defaultDiginfraDockerImage = "diginfra/diginfra-no-driver:master"

const defaultDiginfraDockerEntrypoint = "/usr/bin/diginfra"

var diginfraPriorities = []string{
	"emergency",
	"alert",
	"critical",
	"error",
...
// compareDiginfraPriorities returns 1 if "left" is more urgent than right,
// -1 if "right" is more urgent than left, and 0 otherwise.
func compareDiginfraPriorities(left, right string) int {
	lIndex := math.MaxInt
	rIndex := math.MaxInt
	for i, p := range diginfraPriorities {
