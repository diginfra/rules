...
func getVerRequirement(f *diginfra.PluginVersionRequirementDescription, pluginName string) *diginfra.PluginVersionRequirement {
...
func getRequirements(f *diginfra.PluginVersionRequirementDescription) []*diginfra.PluginVersionRequirement {
	var res []*diginfra.PluginVersionRequirement
...
func findPluginVerRequirement(f *diginfra.RulesetDescription, pluginName string) *diginfra.PluginVersionRequirementDescription {
...
func listNames(f *diginfra.RulesetDescription) []string {
	var names []string
...
func macroNames(f *diginfra.RulesetDescription) []string {
	var names []string
...
func ruleNames(f *diginfra.RulesetDescription) []string {
	var names []string
...
func getCompareOutput(diginfraImage, configFile string, ruleFiles, extraFiles []string) (*diginfra.RulesetDescription, error) {
	testOptions := []diginfra.TestOption{
	...
	for _, rf := range ruleFiles {
		...
		testOptions = append(testOptions, diginfra.WithRules(f))
	}

	if len(configFile) > 0 {
		f := run.NewLocalFileAccessor(configFile, configFile)
		testOptions = append(testOptions, diginfra.WithConfig(f))
	}

	for _, ef := range extraFiles {
		f := run.NewLocalFileAccessor(ef, ef)
		testOptions = append(testOptions, diginfra.WithExtraFiles(f))
	}
	...
	res := diginfra.Test(runner, testOptions...)
...
func compareRulesPatch(left, right *diginfra.RulesetDescription) (res []string) {
	...
	for _, rpr := range right.RequiredPluginVersions {
		var lrl *diginfra.PluginVersionRequirementDescription
	...
	for _, l := range left.Rules {
		for _, r := range right.Rules {
			if l.Info.Name == r.Info.Name {
				...
				if compareDiginfraPriorities(r.Info.Priority, l.Info.Priority) > 0 {
					res = append(res, fmt.Sprintf("Rule `%s` has a more urgent priority than before", l.Info.Name))
				}

				// Adding or removing exceptions for one or more Diginfra rules
...
func compareRulesMinor(left, right *diginfra.RulesetDescription) (res []string) {
...
func compareRulesMajor(left, right *diginfra.RulesetDescription) (res []string) {
	...
	for _, l := range left.Rules {
		for _, r := range right.Rules {
			if l.Info.Name == r.Info.Name {
				...
				if compareDiginfraPriorities(l.Info.Priority, r.Info.Priority) > 0 {
...
var compareCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		leftOutput, err := getCompareOutput(diginfraImage, diginfraConfigPath, leftRules, diginfraFilesPaths)
		if err != nil {
			return err
		}

		rightOutput, err := getCompareOutput(diginfraImage, diginfraConfigPath, rightRules, diginfraFilesPaths)
		if err != nil {
			return err
		}
...
func init() {
	compareCmd.Flags().StringP("diginfra-image", "i", defaultDiginfraDockerImage, "Docker image of Diginfra to be used for validation")
	compareCmd.Flags().StringP("config", "c", "", "Config file to be used for running Diginfra")
	compareCmd.Flags().StringArrayP("file", "f", []string{}, "Extra files required by Diginfra for running")
