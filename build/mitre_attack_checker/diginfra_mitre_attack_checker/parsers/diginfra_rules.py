...
from diginfra_mitre_attack_checker.utils.file import read_yaml


class DiginfraRulesParser(object):
    """
    A Deserialization class for Diginfra rules file in order to define parsing methods
    """
    ...
    def validate(self):
        """
        Simple function to check if the submitted file contains some requirements that Diginfra rules files
        should have.
        """
        error = DiginfraRulesFileContentError(self.path,
                                           message=f"Missing 'required_engine_version' conf in "
                                                   f"{self.path}, so wrong diginfra rules file format or "
                                                   f"not a rules file.")
        try:
    ...
    def get_mitre_relations(self) -> "MitreRelations":
        """
        Build a relation model between techniques and mitre phases described in the diginfra rules
